...
### Postgres — не поддерживается и удалён
**Решение (2026-02):** Поддержка PostgreSQL в коде отсутствует. Эксперимент с бекендом Postgres и параллельной индексацией провели и откатили: для одного процесса SQLite быстрее и проще; выигрыш от Postgres — только в сценарии «общий индекс на несколько процессов». Подробности: [Эксперимент: Postgres и параллельная индексация](docs/postgres-parallel-experiment.md). **Делать поддержку Postgres снова не планируем.**
...
- [ ] **Параллельный парсинг/чтение внутри сегмента** — не делаем (Postgres удалён, см. [postgres-parallel-experiment.md](docs/postgres-parallel-experiment.md)).
