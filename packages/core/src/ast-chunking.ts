...
type SynthParser = {
	...
	traverse?: (
		tree: Tree,
		visitors: Record<string, (ctx: VisitorContext) => undefined | false>,
		options?: any
	) => void
...
function isSemanticBoundary(node: BaseNode, language: string): boolean {
	const boundaryTypes = new Map<string, Set<string>>([
		...
		// HTML/JSX
		['html', new Set(['element', 'comment', 'doctype'])],
		// JSON/YAML
		['json', new Set(['Object', 'Array'])],
		['yaml', new Set(['Document', 'Mapping', 'Sequence'])],
	])
...
function extractContextNodes(tree: Tree, language: string): BaseNode[] {
	const contextTypes = new Map<string, Set<string>>([
		...
		['typescript', new Set(['ImportDeclaration', 'TypeAliasDeclaration', 'InterfaceDeclaration'])],
	])
...
function extractSemanticChunks(
	...
	const lang = language.toLowerCase()
	if (topLevelNodes.length === 1 && (lang === 'javascript' || lang === 'typescript')) {
...
async function loadSynthParser(language: string): Promise<SynthParser | null> {
	try {
		let parserModule: { parse: SynthParser['parse'] }
 ...
 */
async function parseWithSynth(code: string, language: string): Promise<Tree | null> {
...
export const chunkCodeByAST = async (
	...
	const { maxChunkSize = 1000, minChunkSize = 100, preserveContext = true, nodeTypes } = options
